...
def get_benchmark_times(benchmark_res, confidence=0.95):
    ...
    return zip(Ns, times_median, lower_ci_bound, upper_ci_bound)
...
if __name__ == '__main__':
    ...
    benchmark_res_background = benchmark_explainer(background_explainer, data, to_try_N=range(1000, 9000, 1000), num_runs=100)
    ...
    df = pd.DataFrame(bench_times_list, columns=['N', 'median', 'Lower CI', 'Upper CI'])

    # Save the DataFrame to a CSV file
    df.to_csv('bench_times_shap.csv', index=False)